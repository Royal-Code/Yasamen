...
- Referências diretas recomendadas no `.csproj`:
  - `RoyalCode.Razor.Buttons`, caso o componente reutilize `IconButton` para navegação de borda.
  - `RoyalCode.Razor.Styles` para `CssClasses` e classes semânticas.
- `RoyalCode.Razor.Commons` é opcional e só deve entrar se helpers do pacote realmente forem usados.
