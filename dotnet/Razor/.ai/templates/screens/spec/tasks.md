# Tasks - <Nome da Tela>

## Preparação

- [ ] Validar o objetivo da tela e o contexto de uso.
- [ ] Confirmar se a tela é nova ou alteração de tela existente.
- [ ] Confirmar o shell de referência.
- [ ] Confirmar o `Page Pattern` principal.
- [ ] Registrar `As-Is`, quando a tela já existir.

## Modelagem da Tela

- [ ] Definir as zonas funcionais da tela.
- [ ] Selecionar `UIP-*` por zona a partir do catálogo.
- [ ] Justificar compatibilidades secundárias, quando existirem.
- [ ] Registrar estados principais por zona.

## Mapeamento para Yasamen

- [ ] Mapear cada necessidade da tela para `Existente`, `Composição`, `Gap de componente` ou `Gap estrutural`.
- [ ] Identificar componentes Yasamen reaproveitados.
- [ ] Identificar componentes ausentes que exigem specs filhas.
- [ ] Relacionar dependências com `components-plan-list.md` e `ui-plan.md` do roadmap.

## Handoff Técnico

- [ ] Consolidar a cadeia `Shell -> Screen -> Page Pattern -> Zona -> UIP`.
- [ ] Consolidar o mapeamento `UIP -> Yasamen`.
- [ ] Listar specs filhas necessárias em `.ai/specs/`, quando houver.
- [ ] Declarar próximos passos recomendados para implementação da tela.

## Encerramento

- [ ] Comparar a `screen spec` com os guides aplicados.
- [ ] Preencher `delivery.md`.
- [ ] Atualizar o status final da `screen spec`.
- [ ] Fechar ou justificar tasks remanescentes.

