...
**Regra:** a decisão sobre `Style` e `Size` deve ser explícita em componentes novos. Se o componente não suportar variação temática ou de tamanho, o `design.md` precisa justificar por que isso não é cabível. O uso de `Themes`, `Sizes` e dos tokens de `yasamen.css` deve seguir o `styles-and-css.md`.
