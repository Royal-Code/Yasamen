...
export function register(element, listener) {
    ...
    const eventHandler = async (evt) => {
        ...
        // if neither clicked in the element or a target, then prevent default case the first time
        if (!elementClicked && targetsClicked.length === 0) {
            if (registry[listener._id]["_prevented"] === undefined) {
                registry[listener._id]["_prevented"] = true;
                evt.preventDefault();
            }
        }
